...
public class GenerateDBDesignDoc {
	...
	public static String getCodeTypes(Session session, String codetypename) {
		...
		if (type != null) {
			...
			if (list != null && list.size() > 0) {
				...
				ret = "（" + type.getCodetablecnname() + "）"
						+ type.getCodetypeenname() + "\n";
	...
	public static String createDoc() throws Exception {
		...
		Font f = new Font();
		Paragraph p = new Paragraph("深圳市残疾人社会保障与服务信息系统", new Font(Font.NORMAL,
		...
		// 设置中文字体
		BaseFont bfFont = BaseFont.createFont("C:\\Windows\\Fonts\\msyh.ttc,1",
		...
		Font footerFont = new RtfFont("微软雅黑", 12, Font.BOLD,
				Color.BLACK);
		...
		Font contentFontRed = new Font(bfFont, 10, Font.BOLD, new Color(255, 0,
				0));
		...
		for (int i = 0; i < list.size(); i++) {
			...
			for (Leapdatafield d : dlist) {
				...
				p = new Paragraph(d.getIsnullable() == null ? "是" : "否",
						contentFont);
				cell = new Cell(p);
				cell.setHorizontalAlignment(1);
				cell.setVerticalAlignment(1);
				...
				if (d.getCodetype() != null) {
					...
					if (tmp != null) {
						if (maps.containsKey(d.getCodetype())) {
							p = new Paragraph(maps.get(d.getCodetype()),
									contentFontRed);
						} else {
